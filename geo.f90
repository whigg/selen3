...
! Sep 2012: Changed the format 5(f10.4,1x)
!
! +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
...
!  at geodetic sites listed in file GEODETIC_DATABASE. Subsequently, it computes 
...
!INCLUDE "harmonics.f90"
 ...
 !
 COMPLEX*16 CS(JMAX,0:NN+1), CU(JMAX,0:NN+1), CN(JMAX,0:NN+1), CV(JMAX,0:NN+1)
 !
 COMPLEX*16 Y(JMAX,NGEOD), GRD_THETA_Y(JMAX,NGEOD), GRD_LAMBDA_Y(JMAX,NGEOD)
 REAL*8 RATE_S(NGEOD), RATE_N(NGEOD), RATE_UP(NGEOD), RATE_NO(NGEOD), RATE_EA(NGEOD)        
! 
...
!
     ...
     rate_ea(i) = rate_ea(i) + AJUNK*real(((cv(j,nn)-cv(j,nn-1))/DDELTA)*grd_lambda_y(j,i)) 
      ELSEIF(IDER==2) THEN
     rate_s (i) = rate_s (i) + AJUNK*real(((cs(j,nn+1)-cs(j,nn-1))/2D0/DDELTA)*y(j,i))  
     rate_n (i) = rate_n (i) + AJUNK*real(((cn(j,nn+1)-cn(j,nn-1))/2D0/DDELTA)*y(j,i))  
     rate_up(i) = rate_up(i) + AJUNK*real(((cu(j,nn+1)-cu(j,nn-1))/2D0/DDELTA)*y(j,i))  
     rate_no(i) = rate_no(i) + AJUNK*real(((cv(j,nn+1)-cv(j,nn-1))/2D0/DDELTA)*grd_theta_y(j,i))  
     rate_ea(i) = rate_ea(i) + AJUNK*real(((cv(j,nn+1)-cv(j,nn-1))/2D0/DDELTA)*grd_lambda_y(j,i)) 
      ENDIF
!
 2    continue 
!
...
WRITE(82,*)"#	 "
do i=1, ngeod 	
    Write(82,'(3x,i4,2(1x,f8.2),5(f10.4,1x),4x,a22)') & 
