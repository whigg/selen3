...
! Feb 2012: Successful attempt to compuite the "c constant" with the aid of VB 
!
!
! +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
...
!INCLUDE "harmonics.f90"
 ...
 REAL*8, PARAMETER :: DDELTA=DELTA         ! Time increment 
 REAL*8 CDOT, SDOT                         ! dot(c_lm, s_lm) 
 REAL*8 PHILM                              ! A normalization scale 
 COMPLEX*16 RATEN                          ! Complex SC
 CHARACTER*20 date, timc                   ! date and time
 CHARACTER*100 RIGA		           ! A string
 
 COMPLEX*16 N(JMAX,0:NN+1)   ! 4-pi normalized, complex, geoid coefficients
!                            ! Revised February  
!
! +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!
!
! --- Output file for Stokes Coefficients   
...
!--- Reading the 4pi-normalized geoid coefficients...
	open(3,file='shg.bin',status='unknown',form='unformatted') 
        read(3) N
	close(3)
!
...
!--- rate of geoid change in  mm/yr (or m/kyr)  
     ...
     ELSEIF(IDER==2) THEN 
          RATEN = (N(j_index(l,m),NN+1)-N(j_index(l,m),NN-1))/2D0/DDELTA	
     ENDIF
! 
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
! IMPORTANT note:
!
! If you want the "c" constant you need to comment the following two lines. 
! [Verified with VB in Amsterdam February 2012] 
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
!  
!--- now in (yrs)^(-1)
...
!
!
! NEW piece of Fortran script [GRACE-compliant normalization]
! ===========================================================
!
...
!
  		write(1,'(4x,i3,6x,i2,5x,i2,8x,2(E12.6,6x))') & 
