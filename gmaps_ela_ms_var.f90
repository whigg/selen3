...
! Sea level    VARIATION (S)  
! Sea surface  VARIATION (N)  
...
! 
 if(GLOB_ELA_S_VAR==1)  Write(*,*) "    - Computing S at ", np, " pixels "
 if(GLOB_ELA_N_VAR==1)  Write(*,*) "    - Computing N at ", np, " pixels "
 if(GLOB_ELA_U_VAR==1)  Write(*,*) "    - Computing U at ", np, " pixels "
...
!
                IF(GLOB_ELA_S_VAR==1) then 
		        input_filename='svarmap-'//labchar//'.dat'
		        open(101,file=input_filename,status='unknown')
		Endif
                IF(GLOB_ELA_N_VAR==1) then 
		        input_filename='nvarmap-'//labchar//'.dat'
		        open(102,file=input_filename,status='unknown')
		Endif
                IF(GLOB_ELA_U_VAR==1) then 
		        input_filename='uvarmap-'//labchar//'.dat'
...
!
  DO 1 I=1, NP 
!
     IF(GLOB_ELA_S_VAR==1) VARS   = 0D0 
     IF(GLOB_ELA_N_VAR==1) VARN   = 0D0
     IF(GLOB_ELA_U_VAR==1) VARU   = 0D0
!
     SDOT_GIA_CORR=0D0
     NDOT_GIA_CORR=0D0
     UDOT_GIA_CORR=0D0
...
!
     DO 2 J=1, JMAX 
       IF(GLOB_ELA_S_VAR==1) VARS = VARS + ALF(J,ANC(I))*REAL((CS(J,KT))*LONG_TABLE(MM(J),I))
       IF(GLOB_ELA_N_VAR==1) VARN = VARN + ALF(J,ANC(I))*REAL((CN(J,KT))*LONG_TABLE(MM(J),I))
       IF(GLOB_ELA_U_VAR==1) VARU = VARU + ALF(J,ANC(I))*REAL((CU(J,KT))*LONG_TABLE(MM(J),I))
2    CONTINUE
!
! --- South pole
       If     (lat(i)==-90.) then  
       		...
       		do k=1, 4 
			lonx=(k-1)*90.   		
            		if(GLOB_ELA_S_VAR==1) write(101,FMT) lonx, latx, vars  + SDOT_GIA_CORR*DELTA*float(kt)
            		if(GLOB_ELA_N_VAR==1) write(102,FMT) lonx, latx, varn  + NDOT_GIA_CORR*DELTA*float(kt)
            		if(GLOB_ELA_U_VAR==1) write(103,FMT) lonx, latx, varu  + UDOT_GIA_CORR*DELTA*float(kt)
       		enddo
!
! --- North pole
