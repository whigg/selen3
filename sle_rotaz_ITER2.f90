...
!
 CHARACTER*12 HEADER 
 INTEGER I, J, K, L, P, IJ, IS, IT, LJ, MJ, LI, DOM, IND, J21, NANCH, J_INDEX
...
! 
 ...
 COMPLEX*16, ALLOCATABLE :: U_ROT21(:), G_ROT21(:), N_ROT21(:)
...
! --- Allocate memory space
 ...
 ALLOCATE( U_ROT(JMAX,0:NN+1), N_ROT(JMAX,0:NN+1), G_ROT(JMAX,0:NN+1), GMU_ROT(JMAX,0:NN+1) )
 ...
 ALLOCATE( U_LOAD(JMAX,0:NN+1), G_LOAD(JMAX,0:NN+1) )
 ...
 ALLOCATE( U_ROT21(0:NN+1), N_ROT21(0:NN+1), G_ROT21(0:NN+1) )
!
! ********************************************************
! ********************************************************
! ********************************************************
...
!
! ----------------------- ----------------------- ----------------------- ------------------
! ---  EXTERNAL Recursion  --- ---  EXTERNAL Recursion  --- ---  EXTERNAL Recursion  --- ---  
! ----------------------- ----------------------- ----------------------- ------------------
!
!
        Z(:,:,:)=(0D0,0D0)
...
!
       J21=J_INDEX(2,1)
!
            IF(K.EQ.0) PSI_LOAD(K) =  RHOI*IIII(J21,K) + RHOW*Z(J21,K,0)
!	   
            IF(K.GE.1) PSI_LOAD(K) =  RHOI*(IIII(J21,K) -     IIII(J21,K-1))  +  &  	  
                                      RHOW*(   Z(J21,K,0) -   Z(J21,K-1,0)) 
!
            PSI_LOAD(K)= DCONJG(PSI_LOAD(K))*(C0*ERAD**4)/CMA      
!
!
!IF(K.EQ.0) PSI_LOAD(K) =  RHOI* IIII(J21,K)
!IF(K.GE.1) PSI_LOAD(K) =  RHOI*(IIII(J21,K) - IIII(J21,K-1))  	   
!
!if(k.ge.1) write(114,*) " Ice & Zeta", IS, K, RHOI*(IIII(J21,K)  -IIII(J21,K-1))  *(C0*ERAD**4)/CMA  , &
!RHOW*(Z(J21,K,IS-1)-Z(J21,K-1,IS-1))*(C0*ERAD**4)/CMA 	 		   
!if(k.eq.0) write(114,*) " Ice & Zeta", IS, K, RHOI*IIII(J21,K)  *(C0*ERAD**4)/CMA  , &
!RHOW*Z(J21,K,IS-1)*(C0*ERAD**4)/CMA 	 		   
!
!write(111,*) is, k, real(psi_load(k)), aimag(psi_load(k))
!
       ENDDO
!
!
...
!
! ----------------------- ----------------------- ----------------------- ------------------
! ---  INTERNAL Recursion  --- ---  INTERNAL Recursion  --- ---  INTERNAL Recursion  --- ---  
! ----------------------- ----------------------- ----------------------- ------------------
!
...
!
        write(*,*) " ======================= "
        write(*,'(a12,i2,a3,i2)') '     - EXTERNAL step ', it, ' of', SMAX
        write(*,'(a12,i2,a3,i2)') '     - INTERNAL step ', is, ' of', SMAX
!
!
...
!
! --- Updating the H-array by the array Z^{rot} ==================== Recursion
!
...
!
	...
	do k=0, nn+1	
		write(*,*) k, s(j_index(1,0),k), u(j_index(1,0),k), n(j_index(1,0),k) 
	enddo
...
!read(*,*) 
        ...
        do k=0, nn+1	
		write(*,*) k, s(j_index(1,1),k), u(j_index(1,1),k), n(j_index(1,1),k) 
	enddo
...
!
	...
	do k=0, nn+1
		write(*,*) " g direct ", k, G_DIRECT_ROT21(k)
	enddo    
!read(*,*) 
!
        J21=J_INDEX(2,1)
