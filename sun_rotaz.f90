...
!
  END MODULE DATA_FOR_SUB_SUN_ROTAZ
!
!
!
!
!
! ********************************************
       SUBROUTINE SUN_ROTAZ (PSI, IX, SUNROT)
! ********************************************
!
...
!
       ...
       COMPLEX*16 PSI(0:NN+1), SUNROT (0:NN+1)
!
!
! ------------------------------------------------------------------
! Given the Loading excitation function PSI, S_ROTAZ computes the 
! degree 2 - order 1 harmonic component of S^{rot} as a function of
! time  ---- GS November 2012 
!
!       Input:  PSI    (0:NN+1): loading excitation function (input)
!       Output: SunROT (0:NN+1): for IX=1: Degree 2 order 1 component of S^{rot}
!                                for IX=2: Degree 2 order 1 component of U^{rot}
!                                for IX=3: Degree 2 order 1 component of N^{rot}
!                            
! ------------------------------------------------------------------ 
!
  IF(IX.GE.4) then 
...
! --- Viscous components of the Tidal Sea Level Green function
  DO  IP=1, M
     IF(IX==1) GTV(IP) = KT(IP) - HT(IP) 
     IF(IX==2) GTV(IP) =        - HT(IP) 
     IF(IX==3) GTV(IP) = KT(IP)  
  ENDDO
!
! --- Primed elastic term 
  BEP = GTE*AEP 
...
!     
          DO 16 K=0, I    
          SUNROT (I)  = SUNROT (I) + DCONJG(PSI(K)*SFUNC(DELTA*DFLOAT(I-K)))
 16       CONTINUE
!     
        SUNROT (I) = (SIGMA_21/GRA_REF)*SUNROT(I) 
!     
 15   CONTINUE 
!
       END SUBROUTINE SUN_ROTAZ
!
!
...
!
!
  SUBROUTINE READ_DATA_FOR_SUB_SUN_ROTAZ
  USE DATA_FOR_SUB_SUN_ROTAZ
  IMPLICIT NONE 
!
...
!
  CLOSE(13)  
!
  END SUBROUTINE READ_DATA_FOR_SUB_SUN_ROTAZ 
!
!
!
!
