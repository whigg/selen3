...
!       - ebv.dat    ! Horizontal displacement  
...
!EV(nd) = l_e(nd)
!
	EFA(nd) = 0.	                                      ! Agrees with (116) by Tom James
...
!
      	...
      	ESS(l) = float(l)   + 2*h_e(l) - float(l+1)*k_e(l)    ! Agrees with (121) by Tom James 	  
        ESS(l) =-ESS(l)                                       ! Agrees with (121) by Tom James 
!       
! ---- Eustatic solution 
	if(imode==3) then 
			EN(l) = 0.
...
!			
	      BETAS(l,k) =  BETAN(l,k) - BETAU(l,k)   			
!
	      BETAFA(l,k)=  FLOAT(l-1)*BETAN(l,k)                   ! Agrees with (116) by Tom James   			
!			
	      BETASS(l,k)=  2.*BETAU(l,k) - FLOAT(l+1)*BETAN(l,k)   ! Agrees with (121) by Tom James 			
!
              BETASS(l,k) = -BETASS(l,k)                            ! Agrees with (121) by Tom James 
!
	      enddo
	enddo
...
#endif
 Real(qp), parameter :: pi    = 3.1415926535897932384_qp
 END MODULE STRATA 
!
!
!
...
!
   If(i_loading==1) open(3,file='spectrum.dat',status='unknown')
   If(i_loading==0) open(3,file='spectrum_tidal.dat',status='unknown')
!
   If(i_loading==1) open(4,file='ss.dat',     status='unknown')
!
  call DATE_AND_TIME (date,timc)
!
  if(i_loading==1)then
...
!
  ...
  Write(3,*) 'File spectrum.dat, created by Task#1 of TABOO on ', & 
	     ...
	     ' time=', timc(1:2), '.', timc(3:4), '.', timc(5:6)  
  Write(3,*) '1st column: l = Harmonic degree                '
  Write(3,*) '2nd   "   : s, (kyrs**(-1))                    '
  DO l=lmin, lmax 
  do kk=1, nroots 
  	write(3,'(i4,1x,5(e15.7,1x))') l, s(l,kk)     					           
  enddo   
  enddo
  endif
...
!
 IF(IH==1) THEN 
   If(i_loading==1) open(3,file='ih.dat',       status='unknown')
   If(i_loading==1) open(4,file='ihh.dat',      status='unknown')	  
   If(i_loading==0) open(3,file='ih_tidal.dat', status='unknown')
   If(i_loading==0) open(4,file='ihh_tidal.dat',status='unknown')	  
!
...
!
   If(i_loading==1) open(3,file='il.dat',       status='unknown')
   If(i_loading==1) open(4,file='ill.dat',      status='unknown')	  
   If(i_loading==0) open(3,file='il_tidal.dat', status='unknown')
   If(i_loading==0) open(4,file='ill_tidal.dat',status='unknown')	  
!
...
!
 IF(IK==1) THEN 
!
   If(i_loading==1) open(3,file='ik.dat',       status='unknown')
   If(i_loading==1) open(4,file='ikk.dat',      status='unknown')	  
   If(i_loading==0) open(3,file='ik_tidal.dat', status='unknown')
   If(i_loading==0) open(4,file='ikk_tidal.dat',status='unknown')	  
!
...
!
 IF(IH==1) THEN 
!
 If(i_loading==1)  open(3,file='h.dat', status='unknown')
 If(i_loading==1)  open(4,file='hh.dat',status='unknown')	
 If(i_loading==0)  open(3,file='h_tidal.dat', status='unknown')
 If(i_loading==0)  open(4,file='hh_tidal.dat',status='unknown')	  
!
...
!	  
 DO l=lmin, lmax 
!
 if(i_loading==0)write (3, '((i3,1x,96(1x,e20.8)))') l, h_e(l), h_f(l), (h_v (l,k), k = 1, nroots)    
 if(i_loading==1)write (3, '((i3,1x,96(1x,e20.8)))') l, h_e(l), h_f(l), (h_v (l,k), k = 1, nroots)   
!          
 if(l.le.10.or.mod(l,5)==0.or.mod(l,50)==0) &
 write (4, '((i3,1x,96(1x,e20.8)))') &
...
!
 If(i_loading==1)  open(3,file='l.dat', status='unknown')
 If(i_loading==1)  open(4,file='ll.dat',status='unknown')	
 If(i_loading==0)  open(3,file='l_tidal.dat', status='unknown')
 If(i_loading==0)  open(4,file='ll_tidal.dat',status='unknown')	
!
...
!	  
 DO l=lmin, lmax 
!
 if(i_loading==0)write (3, '((i3,1x,96(1x,e20.8)))') l, l_e(l), l_f(l), (l_v (l,k), k = 1, nroots)    
 if(i_loading==1)write (3, '((i3,1x,96(1x,e20.8)))') l, l_e(l), l_f(l), (l_v (l,k), k = 1, nroots)   
!
 if(l.le.10.or.mod(l,5)==0.or.mod(l,50)==0) & 
 write (4, '((i3,1x,96(1x,e20.8)))') &
...
!
 If(i_loading==1)  open(3,file='k.dat', status='unknown')
 If(i_loading==1)  open(4,file='kk.dat',status='unknown')	
 If(i_loading==0)  open(3,file='k_tidal.dat', status='unknown')
 If(i_loading==0)  open(4,file='kk_tidal.dat',status='unknown')	
...
!
!Tidal & Love number 
 if(i_loading==0)write (3, *) l, k_e(l), k_f(l), (k_v (l,k), k = 1, nroots)    
 if(i_loading==1)write (3, '((i3,1x,96(1x,D20.8)))') l, k_e(l), k_f(l), (k_v (l,k), k = 1, nroots)   
!
 if(l.le.10.or.mod(l,5)==0.or.mod(l,50)==0) & 
 write (4, '((i3,1x,96(1x,e20.8)))') &
...
!# Coefficients of the secular polynomial
    ...
    pivo = aa (1) 
!
!# Reporting the coefficients of the secular polynomial     
!
    if(l==2)then 
    OPEN(59,file='CCC.dat',status='unknown') 
    do i=1, nroots +1 
    	write(59,*) aa(i)/aa(1) 	
    	enddo
    Close(59)
...
!# VEC==0 for non physical modes, VEC==1 for 'good' ones 
 Do i = 1, nroots 
    if(abs(rt2(i)) >= 1.e-30_qp .or. rt1(i) >= 0._qp) then
      vec(l,i)= 0._sp  
      else 
      vec(l,i)= 1._sp  
      endif 
...
!
!
    ggg =6.6732e-11_qp    ! Newton constant
!
!
!
!
...
! NV=2   CODE=7 ----> ARCHIE PAULSON APPROACH to VM2....................>>>>>>>>> NEW
!
!
...
!
! 
rho(1) = (4.41241_qp+4.38071_qp)/2._qp*1.e3_qp
rho(2) = (3.99214_qp+3.98399_qp)/2._qp*1.e3_qp
!
             Endif 
!
!
...
r (4)      = 6371._qp; rho(4)=4120._qp ; rmu(4)=0.728_qp     ! Litosphere  
!
endif 
...
!
!
rho(2) = (4.41241_qp+4.38071_qp)/2._qp*1.e3_qp
rho(3) = (3.54325_qp+3.51639_qp)/2._qp*1.e3_qp  
!
endif 
!
!                      
...
!
rmu (0) = 0._qp  
rho (1) = (4.41241+4.38071)/2._qp*1.e3_qp   ! Lower mantle density
!
endif 
!
!
...
rho (1) = (4.41241+4.38071)/2._qp*1.e3_qp ! Lower mantle rigidity
!
endif 
!
!
...
!
!
if(lt .ne. (r(nv+1)-r(nv))/1e3_qp .and. iv==0) then
...
if(lt .ne. (r(nv+1)-r(nv))/1e3_qp .and. iv==1) then 
...
!
          Write(99,'(a51,F12.4)') &
'Lithospheric thickness effectively employed (km) =', (r(nv+1)-r(nv))/1e3_qp 
