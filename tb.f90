...
!       - ebv.dat    ! Horizontal displacement Green function
!
! =============================================================================
!
!
 ...
 INTEGER, PARAMETER :: L1=1, L2=LMAX
...
!
!
!WRITE(*,*) '    - beta.f: reading the residues h/s and k/s and relaxation times...'
!
...
!
   do l=2, LMAX
      	READ (1,'((i3,1x,24(1x,e20.8)))') p, h_e
      	READ (2,'((i3,1x,24(1x,e20.8)))') p, k_e
      	READ (3,'((i3,1x,24(1x,e20.8)))') p, l_e
!
      	E(l)=(1.0+k_e-h_e)/(2.0*float(l)+1.0)
!	
	EU(l)=h_e      /(2.0*float(l)+1.0)
!
	EV(l)=l_e      /(2.0*float(l)+1.0)
!
	if(imode==3) then 
			E(l) =0.
			EU(l)=0.
	...
	if(imode==4) then 
			E(l)=(1.0+0.0-0.0)/(2.0*float(l)+1.0)  
			EU(l)=0.0/(2.0*float(l)+1.0) 
			EV(l)=0.0/(2.0*float(l)+1.0) 
		     endif
	if(imode==6) then 
		        E(l)=(1.0+k_e-h_e)/(2.0*float(l)+1.0)
			EU(l)=h_e/(2.0*float(l)+1.0) 
	...
	if(imode==7) then 
		        E(l)=(0.0+0.0-h_e)/(2.0*float(l)+1.0)  
			EU(l)=h_e/(2.0*float(l)+1.0) 
			EV(l)=l_e/(2.0*float(l)+1.0) 
		     endif		
   end do
...
!
!   
! --- If no ice is selected, the E and BETA arrays are not 
!     computed, being related with the ice time-history 
!
   IF(NN/=0) THEN     
!
!WRITE(*,*) '    - Computing the E and  BETA arrays...'
!
        BETA(0,:)=0.0
        BETA(1,:)=0.0
!
...
!
!   	
   	open(1,file= 'eb.dat',status='unknown')
   	open(2,file='ebu.dat',status='unknown')
   	open(3,file='ebv.dat',status='unknown')
!  
	do l=0, lmax
		write(1,*) l, E(l),  (beta (l,k), k=0, nn)
		write(2,*) l, EU(l), (betau(l,k), k=0, nn)
		write(3,*) l, EV(l), (betav(l,k), k=0, nn)
	enddo
!
	close(1)
	close(2)
