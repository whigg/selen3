...
! the couple (lon,lat) for a given site, in REAL*4 format. GS 06/10/2007 - 
!
! Fixed a significant BUG here on May 2010. There was a mistake in the conversion. 
!
! 
  IMPLICIT NONE 
...
!
  ...
  INTEGER L, LMAX 
  REAL*8 LAT
  REAL*8 LEGP(0:LLMAX), P(1:LLMAX+1)
  REAL*8 COSDD
!
...
! REAL*4 precision - Note that here the degree 0 is at place '0' in the array PLEG. 
...
!
  ...
  elseif(lmax>llmax) then 
		Write(88,*) "Error in Sbr. PLEG: The degree exceeds 256", lmax 
	 	Write(88,*) "The program will STOP ----------------"
 	        Write(* ,*) "Error in Sbr. PLEG: The degree exceeds 256", lmax 
	 	Write(* ,*) "The program will STOP ----------------"
                call stop_config 
	        Stop 2		
  Endif  
...
!
  ...
  elseif(lmax>256) then 
		Write(88,*) "Error in Sbr. HARMO: The degree exceeds 256", lmax 
	 	Write(88,*) "The program will STOP ----------------"
 	        Write(* ,*) "Error in Sbr. HARMO: The degree exceeds 256", lmax 
	 	Write(* ,*) "The program will STOP ----------------"
                call stop_config 
  Endif   
!  
...
!
  ...
  elseif (lon<0.or.lon>360.) then
	 	...
	 	Write(* ,*) "The program will STOP ----------------"
	        ...
	        Stop 2	
  elseif(lmax>256) then 
		Write(88,*) "Error in Sbr. GRAD_THETA_HARMO: The degree exceeds 256", lmax 
	 	Write(88,*) "The program will STOP ----------------"
 	        Write(* ,*) "Error in Sbr. GRAD_THETA_HARMO: The degree exceeds 256", lmax 
	 	Write(* ,*) "The program will STOP ----------------"
                call stop_config 
  Endif   
!  
...
!
  ...
  elseif (lon<0.or.lon>360.) then
	 	...
	 	Write(* ,*) "The program will STOP ----------------"
	        ...
	        Stop 2	
  elseif(lmax>256) then 
		Write(88,*) "Error in Sbr. GRAD_LAMBDA_HARMO: The degree exceeds 256", lmax 
	 	Write(88,*) "The program will STOP ----------------"
 	        Write(* ,*) "Error in Sbr. GRAD_LAMBDA_HARMO: The degree exceeds 256", lmax 
	 	Write(* ,*) "The program will STOP ----------------"
                call stop_config 
  Endif   
!  
...
!
! *** Revised GS July 2010 - g95 - Double precision implementation
!     Modified to REAL*8 by GS september 30, 2011
!
  IMPLICIT NONE
  INTEGER L, M 
  COMMON /AREA_DEG/L, M
...
!
! PLM (double) 
  call plmbar_mod (l, 90.0-180.*xx/pi, plm)
!
...
! --- polar pixel 
          	if(x==0.d0.and.y==0.d0) then 
          		lonpx=0.
                	if(z>=0.d0) latpx = +90.d0
                	if(z<=0.d0) latpx = -90.d0
! --- ordinary pixel 
	  		else                         	  
	  		lonpx = atan2(y,x)*180.d0/pi 
          		if (lonpx < 0.d0) lonpx=360.d0+ lonpx
          		latpx = 90.d0-acos(z)*180.d0/pi 
          	endif
	 lonp(j+1)=lonpx 
	 latp(j+1)=latpx 	
!
...
!    The maximum number of ice elements is now increased to 90,000 -       
!
! ---------------------------------------------------------------------------
!
   IMPLICIT NONE 
   INTEGER, PARAMETER :: SIZE  = 1000
   INTEGER, PARAMETER :: NMAX  = 9E4
!
...
!
!
   SUBROUTINE HARMO_BREAKER (NDATA, SIZE, SLOTS, LOW, HIG)
!
! +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
! Copyright (C) 2008 Giorgio Spada, Florence Colleoni, and Paolo Stocchi 
!
...
!INTEGER, PARAMETER :: SIZE  = 1000
   INTEGER, PARAMETER :: NMAX  = 9E4
!
   INTEGER S, SIZE, NDATA, SLOTS, SMIN, SMAX, LOW(1), HIG(1)
!         
! ---------------------------------------------------------------------------
!
   If(NDATA.GT.NMAX)  then 
